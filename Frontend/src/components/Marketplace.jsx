    ...
    <div className="max-w-sm border border-neutral-800 bg-neutral-900 rounded-lg overflow-hidden hover:shadow-lg hover:shadow-neutral-800 transition-all duration-300">
      ...
      <div className="px-4 py-3">
        <div className="flex justify-between items-center border-b border-neutral-700 pb-2">
          <span className="text-xl font-semibold text-blue-500">₹{data.cost}</span>
          <span className="text-neutral-400 text-sm">{data.date}</span>
        </div>
        <div className="mt-2 mb-4 h-12">
          <p className="text-neutral-300 text-sm line-clamp-2">{data.description}</p>
        </div>
        <button
...
const itemsData = [
  ...
  {
    images: [
      "https://i.ebayimg.com/images/g/UXMAAOSwrrplv7n6/s-l1600.jpg",
    ],
    cost: "1500",
    description:
        ...
        <div className="h-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            {items.map((data, index) => (
              <ItemCard data={data} key={index} onViewDetails={handleViewDetails} />
            ))}
          </div>
        </div>
      </div>
